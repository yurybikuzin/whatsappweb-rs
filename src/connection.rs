...
use super::{
    Chat, ChatAction, Contact, GroupMetadata, GroupParticipantsChange, Jid, MediaType,
    PresenceStatus,
};
use crypto;
use errors::*;
...
use node_protocol;
use node_protocol::{AppEvent, AppMessage, GroupCommand, MessageEventType, Query};
use node_wire::Node;
use timeout;
use websocket_protocol::{WebsocketMessage, WebsocketMessageMetric, WebsocketMessagePayload};
...
pub trait WhatsappWebHandler<H = Self>
where
    H: WhatsappWebHandler<H> + Send + Sync + 'static,
{
    ...
    fn on_message(
        &self,
        connection: &WhatsappWebConnection<H>,
        message_new: bool,
        message: Box<WhatsappMessage>,
    );
}
...
enum SessionState {
    PendingNew {
        ...
        public_key: Vec<u8>,
        client_id: [u8; 8],
        qr_callback: Box<dyn Fn(QrCode) + Send>,
    },
    PendingPersistent {
...
impl<H: WhatsappWebHandler<H> + Send + Sync + 'static> WhatsappWebConnectionInner<H> {
    fn send_json_message(
        &mut self,
        message: JsonValue,
        cb: Box<dyn Fn(JsonValue, &WhatsappWebConnection<H>) + Send>,
    ) {
        debug!("sending json {:?}", &message);
        let tag = self.alloc_message_tag();
        self.ws_send_message(
            WebsocketMessage {
                tag: tag.into(),
    ...
    fn send_app_message(
        &mut self,
        tag: Option<String>,
        metric: WebsocketMessageMetric,
        app_message: AppMessage,
        cb: Box<dyn Fn(WebsocketResponse, &WhatsappWebConnection<H>) + Send>,
    ) {
        self.epoch += 1;
    ...
    fn ws_send_message(
        ...
        callback: Box<dyn Fn(WebsocketResponse, &WhatsappWebConnection<H>) + Send>,
    ) {
    ...
    fn send_binary_message(
        ...
        let encrypted_message = if let SessionState::Established {
            ref persistent_session,
        } = self.session_state
        {
            crypto::sign_and_encrypt_message(
                &persistent_session.enc,
                &persistent_session.mac,
                &message,
            )
        } else {
            return;
    ...
    fn decrypt_binary_message(&mut self, encrypted_message: &[u8]) -> Result<Vec<u8>> {
        ...
        {
            crypto::verify_and_decrypt_message(
                &persistent_session.enc[..],
                &persistent_session.mac[..],
                &encrypted_message,
            )
    ...
    fn handle_server_conn(
        ...
        let (new_session_state, persistent_session, user_jid) = match self.session_state {
            SessionState::PendingNew {
                ...
                let (enc, mac) =
                    crypto::calculate_secret_keys(&secret, private_key.take().unwrap())?;
            ...
            _ => {
                bail! {"Session already established but received conn packet"}
            }
        };
            ...
            match timeout_manager.on_timeout(event) {
                Some(timeout::TimeoutState::Normal) => {
                    ...
                    timeout_manager.arm(
                        ...
                        timeout::TimeoutState::Deathline,
                    );
                }
    ...
    fn handle_server_challenge(&mut self, challenge: &[u8]) {
        let message = if let SessionState::PendingPersistent {
            ref persistent_session,
        } = self.session_state
        {
    ...
    fn ws_on_connected(&mut self, out: Sender) {
        ...
        let message: (
            JsonValue,
            Box<dyn Fn(JsonValue, &WhatsappWebConnection<H>) + Send>,
        ) = match self.session_state {
            SessionState::PendingNew { ref client_id, .. } => {
                let init_command =
                    json_protocol::build_init_request(base64::encode(&client_id).as_str());

                (
                    init_command,
                    Box::new(move |response, connection| {
            ...
            SessionState::PendingPersistent {
                ...
                let init_command = json_protocol::build_init_request(
                            ...
                            let message: (
                                ...
                                SessionState::PendingPersistent {
                                    ...
                                    (
                                        ...
                                        Box::new(move |response, connection| {
                                            if let Err(err) =
                                                json_protocol::parse_response_status(&response)
                                            {
                                                error!("error {:?}", err);
                                                connection.ws_disconnect();
                                                connection
                                                    .handler
                                                    .on_disconnect(DisconnectReason::Removed);
                                            }
                                        }),
                                    )
                                }
...
impl<H: WhatsappWebHandler<H> + Send + Sync> WhatsappWebConnection<H> {
    fn new<Q: Fn(QrCode) + Send + 'static>(
        qr_callback: Box<Q>,
        handler: H,
    ) -> WhatsappWebConnection<H> {
        let mut client_id = [0u8; 8];
        SystemRandom::new().fill(&mut client_id).unwrap();

        let (private_key, public_key) = crypto::generate_keypair();

        println!("will WhatsappWebConnection");
        WhatsappWebConnection {
    ...
    fn with_persistent_session(
        ...
        WhatsappWebConnection {
            ...
            inner: Arc::new(Mutex::new(WhatsappWebConnectionInner {
                ...
                requests: HashMap::new(),
                messages_tag_counter: 0,
                session_state: SessionState::PendingPersistent { persistent_session },
                epoch: 0,
            })),
        }
    }
    ...
    fn send_app_message(
        ...
        self.inner
            .lock()
            .unwrap()
            .send_app_message(tag, metric, app_message, cb)
    }
    ...
    fn ws_on_message(&self, message: &Message) {
        ...
        match message.payload {
                ...
                if let Some(cb) = inner.requests.remove(message.tag.deref()) {
                    ...
                    match ServerMessage::deserialize(&payload) {
                        Ok(ServerMessage::ConnectionAck {
                            ...
                            if let Ok((persistent_session, user_jid)) = inner.handle_server_conn(
                                user_jid,
                                client_token,
                                server_token,
                                secret,
                                ...
                                self.handler
                                    .on_persistent_session_data_changed(persistent_session);
                                self.handler
                                    .on_user_data_changed(&self, UserData::UserJid(user_jid));
                            }
                        }
                        ...
                        Ok(ServerMessage::MessageAck {
                            ...
                            participant,
                            time,
                        }) => self.handler.on_user_data_changed(
                            self,
                            UserData::MessageAck(MessageAck::from_server_message(
                                message_id,
                                level,
                                sender,
                                receiver,
                        ...
                        Ok(ServerMessage::GroupIntroduce {
                            newly_created,
                            inducer,
                            meta,
                        }) => {
                        ...
                        Ok(ServerMessage::GroupParticipantsChange {
                            group,
                            change,
                            inducer,
                            participants,
                            ...
                            self.handler.on_user_data_changed(
                                self,
                                UserData::GroupParticipantsChange {
                                    group,
                                    change,
                                    inducer,
                                    participants,
            ...
            WebsocketMessagePayload::BinarySimple(encrypted_payload) => {
                let payload =
                    Node::deserialize(&inner.decrypt_binary_message(encrypted_payload).unwrap())
                        .unwrap();
                ...
                if let Some(cb) = inner.requests.remove(message.tag.deref()) {
                    ...
                    match AppMessage::deserialize(payload) {
                        Ok(AppMessage::Contacts(contacts)) => {
                            drop(inner);
                            self.handler
                                .on_user_data_changed(self, UserData::ContactsInitial(contacts));
                        }
                        Ok(AppMessage::Chats(chats)) => {
                            drop(inner);
                            self.handler
                                .on_user_data_changed(self, UserData::Chats(chats));
                        }
                        Ok(AppMessage::MessagesEvents(event_type, events)) => {
                            drop(inner);
                            for event in events {
                                match event {
                                    AppEvent::Message(message) => self.handler.on_message(
                                        self,
                                        event_type == Some(MessageEventType::Relay),
                                        message,
                                    ),
                                    AppEvent::MessageAck(message_ack) => {
                                        self.handler.on_user_data_changed(
                                            self,
                                            UserData::MessageAck(message_ack),
                                        )
                                    }
                                    AppEvent::ContactDelete(jid) => self
                                        .handler
                                        .on_user_data_changed(self, UserData::ContactDelete(jid)),
                                    AppEvent::ContactAddChange(contact) => {
                                        self.handler.on_user_data_changed(
                                            self,
                                            UserData::ContactAddChange(contact),
                                        )
                                    }
                                    AppEvent::ChatAction(jid, action) => {
                                        self.handler.on_user_data_changed(
                                            self,
                                            UserData::ChatAction(jid, action),
                                        )
                                    }
                                    AppEvent::Battery(level) => self
                                        .handler
                                        .on_user_data_changed(self, UserData::Battery(level)),
    ...
    pub fn send_message_played(&self, id: MessageId, peer: Peer) {
        let mut inner = self.inner.lock().unwrap();
        inner.epoch += 1;
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::MessagePlayed { id, peer }],
        );
        self.send_app_message(
            None,
            WebsocketMessageMetric::Received,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn send_message_read(&self, id: MessageId, peer: Peer) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::MessageRead { id, peer }],
        );
    ...
    pub fn set_presence(&self, presence: PresenceStatus, jid: Option<Jid>) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::PresenceChange(presence, jid)],
        );
        self.send_app_message(
            None,
            WebsocketMessageMetric::Presence,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn set_status(&self, status: String) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::StatusChange(status)],
        );
        self.send_app_message(
            None,
            WebsocketMessageMetric::Status,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn set_notify_name(&self, name: String) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::NotifyChange(name)],
        );
        self.send_app_message(
            None,
            WebsocketMessageMetric::Profile,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn block_profile(&self, unblock: bool, jid: Jid) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::BlockProfile { unblock, jid }],
        );
        self.send_app_message(
            None,
            WebsocketMessageMetric::Block,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn send_chat_action(&self, action: ChatAction, chat: Jid) {
        let msg = AppMessage::MessagesEvents(
            Some(MessageEventType::Set),
            vec![AppEvent::ChatAction(chat, action)],
        );
    ...
    pub fn send_message(&self, message_content: ChatMessageContent, jid: Jid) {
        ...
        let msg = AppMessage::MessagesEvents(
            ...
            vec![AppEvent::Message(Box::new(WhatsappMessage {
        ...
        self.send_app_message(
            Some(message_id.0),
            WebsocketMessageMetric::Message,
            msg,
            Box::new(|_, _| {}),
        );
    }
    ...
    pub fn group_create(&self, subject: String, participants: Vec<Jid>) {
        self.inner
            .lock()
            .unwrap()
            .send_group_command(GroupCommand::Create(subject), participants);
    }
    ...
    pub fn group_participants_change(
        ...
        self.inner.lock().unwrap().send_group_command(
    ...
    pub fn get_messages_before(
        ...
        id: String,
        count: u16,
        callback: Box<dyn Fn(Option<Vec<WhatsappMessage>>) + Send + Sync>,
    ) {
        let msg = AppMessage::Query(Query::MessagesBefore { jid, id, count });
        self.send_app_message(
            None,
            WebsocketMessageMetric::QueryMessages,
            msg,
            Box::new(move |response, _| match response {
    ...
    pub fn request_file_upload(
        ...
        callback: Box<dyn Fn(Result<&str>) + Send + Sync>,
    ) {
        self.send_json_message(
            json_protocol::build_file_upload_request(hash, media_type),
    ...
    pub fn get_profile_picture(
        ...
        self.send_json_message(
            json_protocol::build_profile_picture_request(jid),
            Box::new(move |response, _| {
    ...
    pub fn get_profile_status(&self, jid: &Jid, callback: Box<dyn Fn(Option<&str>) + Send + Sync>) {
        self.send_json_message(
            json_protocol::build_profile_status_request(jid),
            Box::new(move |response, _| {
    ...
    pub fn get_group_metadata(
        &self,
        jid: &Jid,
        callback: Box<dyn Fn(Option<GroupMetadata>) + Send + Sync>,
    ) {
        debug_assert!(jid.is_group);
        self.send_json_message(
            json_protocol::build_group_metadata_request(jid),
            Box::new(move |response, _| {
    ...
    pub fn subscribe_presence(&self, jid: &Jid) {
        self.send_json_message(
            json_protocol::build_presence_subscribe(jid),
            Box::new(|_, _| {}),
        );
    }
...
impl<H: WhatsappWebHandler<H> + Send + Sync + 'static> Handler for WsHandler<H> {
    fn build_request(&mut self, url: &Url) -> ws::Result<Request> {
        ...
        request
            .headers_mut()
            .push(("Origin".to_string(), b"https://web.whatsapp.com".to_vec()));
        Ok(request)
    }
    ...
    fn on_new_timeout(&mut self, event: Token, timeout: Timeout) -> ws::Result<()> {
        if let WebsocketState::Connected(_, ref mut timeout_manager) = self
            .whatsapp_connection
            .inner
            .lock()
            .unwrap()
...
pub fn with_persistent_session<H: WhatsappWebHandler<H> + Send + Sync + 'static>(
    ...
    let whatsapp_connection =
        WhatsappWebConnection::with_persistent_session(persistent_session, handler);
