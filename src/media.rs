...
pub fn generate_thumbnail_and_get_size(image: &[u8]) -> (Vec<u8>, (u32, u32)) {
    ...
    JPEGEncoder::new(&mut thumbnail_writter)
...
pub fn upload_file<H>(
    file: &[u8],
    media_type: MediaType,
    connection: &WhatsappWebConnection<H>,
    callback: Box<Fn(Result<FileInfo>) + Send + Sync>,
) where
    H: WhatsappWebHandler + Send + Sync + 'static,
{
    ...
    connection.request_file_upload(
        &file_hash.clone(),
        media_type,
        Box::new(move |url: Result<&str>| {
            match url {
                Ok(url) => {
                    ...
                    thread::spawn(move || {
                        let form = reqwest::multipart::Form::new()
                            ...
                            .part(
                                "file",
                                reqwest::multipart::Part::reader(Cursor::new(
                                    file_encrypted.to_vec(),
                                ))
                        ...
                        let file_info = reqwest::Client::new()
                            .post(url.as_str())
                            .multipart(form)
                            .send()
                            ...
                            .and_then(|response| {
                                json::parse(&response)
                                    .map_err(|e| (Error::with_chain(e, "invalid response")))
                            })
